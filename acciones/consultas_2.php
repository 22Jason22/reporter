...
function obtenerConteoPorSexo($conexion) {
    $sql = "SELECT sexo, COUNT(*) AS total FROM trabajadas GROUP BY sexo"; // Asegúrate de que el nombre de la tabla sea correcto
    $resultado = $conexion->query($sql);
...
function obtenerConteoPorTipoSujeto($conexion) {
    $sql = "SELECT tipo_sujeto, COUNT(*) AS total FROM trabajadas GROUP BY tipo_sujeto ORDER BY total DESC"; // Consulta SQL
    $resultado = $conexion->query($sql);
